	...
	private InteractionSequence mapInteractionSeqModelObjToDTO(BindingSet modelObj) {
	    ...
	    RDFRepositoryManager repManager = new RDFRepositoryManager(Parametrization.GRAPHDB_SERVER);                       
            String query= String.format( header +
                    ...
                    + "  {?subu a :User. \r\n"
                    + "   ?subu :makes ?i.  ?i :occurs_in ?c .  ?c :value ?contextvalue. \r\n"
                    + "   filter(%s)} GROUP BY ?subu  } \r\n"
                    +"    \r\n"
                    +"    FILTER (?u1 = ?subu && (%s)) \r\n"
                    +"} \r\n"    
                    +"GROUP BY ?u1 ?s ?q  ?iri \r\n"
        ...
        public List<Subservice> getSubserviceRecommendationCosineDistance(int k, String user){
            ...
            String query =  String.format(header            
                +"select  ?idSubservice ?name (sum(?valueR*?cosine_d) as ?score) \r\n"
                +"where{ \r\n"
                +"?u2 :uses ?subservice. \r\n"
                +"?r :is_from_a ?u2. \r\n"
                +"?r :is_for_a ?subservice. \r\n"
                +"?r :valueRating ?valueR. \r\n"
                +"?subservice :id ?idSubservice. \r\n"
                +"?subservice :name ?name. \r\n"
                +"\r\n"
                +"{select ?u1 ?u2 ?sqrtV1 ?v1pow2 ?sqrtV2  ?v2pow2  ?dot \r\n"
                +"((?dot / (?sqrtV2 * ?sqrtV1))as ?cosine_d) \r\n"
                +"where{    \r\n"
                +"     BIND (ofn:sqrt(?v2pow2) AS ?sqrtV2).\r\n"
                +"     BIND (ofn:sqrt(?v1pow2) AS ?sqrtV1).  \r\n"
                +"    {select ?u1  ?u2 (count(*) as ?count_r) \r\n"
                +"     (sum(?value1*?value2) as ?dot) (sum(?value1*?value1) as ?v1pow2) (sum(?value2*?value2) as ?v2pow2)  \r\n"
                +"     where { \r\n"
